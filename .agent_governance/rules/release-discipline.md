...
## Law II-H — Release PR Must Be Docs-Free (Split on Audit Violation)

The PR Resolution Audit enforces two invariants:
1. **Blast radius ≤ 5 top-level directories** (counted by `awk -F/ '{print $1}'` — root files like `README.md` each count as one).
2. **Docs isolation** — `README.md` and `docs/*.md` may NOT appear in the same PR as engine or workflow changes.

A release PR generated by `just release` always touches: `Cargo.toml`, `Cargo.lock`, `crates/` (SBOMs + code), `tools/` (SBOMs) — that is 4 top-level entries. Adding `README.md` and `docs/index.md` bumps it to 6 and also triggers the docs-isolation violation.

**Required procedure for every release PR:**
1. After `just release` creates and pushes the release branch, strip `README.md` and `docs/index.md` from that branch with an additional commit:
   ```bash
   git show origin/main:README.md > README.md
   git show origin/main:docs/index.md > docs/index.md
   git add README.md docs/index.md
   git commit -m "chore: strip docs from release PR (will land in separate docs PR)"
   git push origin release/vX.Y.Z
   ```
2. Create a `docs/vX.Y.Z` branch off main, apply the version bump, open a docs-only PR:
   ```bash
   git checkout -b docs/vX.Y.Z main
   sed -i 's/vOLD/vNEW/g' README.md docs/index.md
   git add README.md docs/index.md
   git commit -m "docs: bump version to vX.Y.Z in README and index"
   git push -u origin docs/vX.Y.Z
   gh pr create --title "docs: bump version to vX.Y.Z" ...
   ```

**Verification before opening any release PR:**
```bash
gh pr diff <PR> --name-only | awk -F/ 'NF {print $1}' | sort -u | wc -l
# Must be ≤ 5
gh pr diff <PR> --name-only | grep -E '^(README\.md|docs/)'
# Must be empty for release PRs
```

**Root cause (Sprint 170, PR #140):** Release commit included `README.md` + `docs/index.md` version bumps. PR Resolution Audit counted 6 top-level dirs and flagged docs-engine mixing. CI blocked until the two files were stripped into a separate PR (#142).
